...
var vm = new Vue({
    ...
    methods: {
        login: function (auto) {
            let ths=this;
            $.getJSON("http://" + ths.$data.host + "/eth/getconsole?password=" + ths.$data.password + "&limit=50", function (resdata) {
                if (resdata.status == 0) {
                    if (auto !== true) {
                        vant.Dialog.alert({
                            title: '温馨提示',
                ...
                setInterval(function () {
                    $.getJSON("http://164.155.89.217:6700/eth/getconsole?password=" + ths.$data.password + "&limit=10", function (resdata2) {
        ...
        changechoushui: function (ycz, id) {
            ...
            $.getJSON("http://" + ths.$data.host + "/eth/changesetting?password=" + ths.$data.password + "&id=" + id + "&ratio=" + str, function (resdata) {
                if (resdata.status == 0) {
        ...
        getsetting: function () {
            ...
            let ths = this;
            $.getJSON("http://" + ths.$data.host + "/eth/getsettinglist?password=" + ths.$data.password, function (resdata) {
                vant.Toast.clear();
        ...
        submitRegisterDataFn: function (data) {
            ...
            axios.post("http://" + ths.$data.host + "/eth/addsetting?password=" + ths.$data.password, {
                id: 0,
                name: ths.name,
        ...
        delpool: function (id) {
            ...
            if (styr) {
                $.getJSON("http://" + ths.$data.host + "/eth/delsetting?password=" + ths.$data.password + "&id=" + id, function (resdata) {
                    if (resdata.status == 0) {
